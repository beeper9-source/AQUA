-- Supabase 스키마 완전 수정: winner -> result 컬럼 변경 및 courts 테이블 복원

-- 1. matches 테이블의 winner 컬럼을 result로 변경 (아직 변경되지 않은 경우)
DO $$ 
BEGIN
    -- winner 컬럼이 존재하는지 확인
...
-- 2. sets 테이블의 winner 컬럼을 result로 변경 (아직 변경되지 않은 경우)
DO $$ 
BEGIN
    -- winner 컬럼이 존재하는지 확인
    IF EXISTS (SELECT 1 FROM information_schema.columns 
...
-- 10. 완료 메시지
SELECT '스키마 수정이 완료되었습니다! winner -> result 변경 및 courts 테이블 복원 완료' as message;
